    ...
    while True:
        ...
        if function_choice == '1':
            ...
            if export_info == '1':
                ...
                saving_event_info.save_event_info(function_name, events, token)
            ...
            if export_info == '2':
                ...
                events = getting_info.search_events(event_name, date_start, date_end, place, moderation, token)
                saving_event_info = sei.SavingEventInfo(function_name, events)
                saving_event_info.save_event_info(function_name, events, token)
                print(Fore.GREEN + 'Файл сохранён в корневую папку')
            ...
            if not company_events:
                ...
                saving_event_info.save_event_info(function_name, company_events, token)
                print(Fore.GREEN + 'Файл сохранён в корневую папку')
            ...
            saving_users_info.save_user_info(function_name, ids, token)
            print(Fore.GREEN + 'Файл сохранён в корневую папку')
            ...
            token = auth.Auth.log_in()
            if not token:
                auth.Auth.send_failed_email(name, token, ip)
                break
            else:
                print()
                print('Авторизация прошла успешно. Продолжаем выгружать информацию.')
                print()
                ip = auth.Auth.get_ip()
                auth.Auth.send_success_email(name, token, ip)
            ...
            data = getting_info.get_participants(events_id, token)
            saving_users_info = sui.SavingUsersInfo(function_name, data)
            saving_users_info.save_participants_info(function_name, data, token)
